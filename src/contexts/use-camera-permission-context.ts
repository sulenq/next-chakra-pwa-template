    ...
    updateCameraPermissionsStatus: async () => {
      ...
      try {
        if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
          ...
          if (hasActiveAccess) {
            const isPermanent = getStorage("perm_camera_permanent", "local") === "true";
            set({
              cameraPermissionsStatus: isPermanent ? "granted_permanent" : "granted_temporary",
            });
            return;
          }
        }
      ...
      const cachedSession = getStorage("perm_camera", "session") as UnifiedPermissionState | null;
...
if (isClient()) {
  setTimeout(() => {
    useCameraPermission.getState().updateCameraPermissionsStatus();
  }, 0);
}
